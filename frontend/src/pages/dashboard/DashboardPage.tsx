...
import { Server, Play, Activity, DollarSign, ArrowRight, Clock, CheckCircle, XCircle, Database, HeartPulse } from 'lucide-react'
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const ALL_SECTIONS = ['pinned_services', 'stats', 'quick_links', 'health', 'recent_jobs']
...
export default function DashboardPage() {
  ...
  const sectionOrder = usePreferencesStore((s) => s.preferences.dashboard_sections.order)
  const reorderSections = usePreferencesStore((s) => s.reorderSections)
  ...
  // Handle new sections not in saved order
  const effectiveOrder = [
    ...sectionOrder.filter((s) => ALL_SECTIONS.includes(s)),
    ...ALL_SECTIONS.filter((s) => !sectionOrder.includes(s)),
  ]
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (over && active.id !== over.id) {
      ...
      reorderSections(arrayMove(effectiveOrder, oldIndex, newIndex))
    }
  }

  const sectionRenderers: Record<string, () => React.ReactNode> = {
    pinned_services: () => <PinnedServices key="pinned_services" />,
    stats: () => (
      <DashboardSection key="stats" id="stats" title="Statistics">
    ...
    quick_links: () =>
      quickLinks.length > 0 ? (
        <DashboardSection key="quick_links" id="quick_links" title="Quick Links">
